...
@Slf4j
public class GetMunicipalityByIdHandler {

    private final MunicipalityRepository repository;
    private final SecurityPort securityPort;
    private final Guard<Municipality> guard;
    ...
    public Municipality handle(GetMunicipalityByIdQuery query) {
        ...
        var municipality = repository
            .findById(query.municipalityId())
            .orElseThrow(() -> new NotFoundException("province.msg.not-found", query.municipalityId(), Municipality.class));
        guard.checkRead(municipality, user);
        return municipality;
    }
